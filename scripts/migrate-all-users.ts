...
async function main() {
  ...
  // Get users that are missing from the new database
  const [missingUsers] = await legacyConnection.execute(`
    SELECT * FROM users 
    WHERE id NOT IN (SELECT id FROM fitqvibes.User)
  ...
  for (let i = 0; i < missingUsers.length; i += batchSize) {
      ...
      try {
        ...
        if (user.trainer_unlocked === 1) {
          ...
          try {
            ...
            console.log(`Created trainer record for user ${user.id}: ${user.name}`)
          } catch (e: any) {
    ...
    console.log(`Progress: ${Math.min(i + batchSize, missingUsers.length)} / ${missingUsers.length} (${successCount} migrated, ${errorCount} errors)`)
  }

  // Reset auto increment to max ID + 1
  const [maxId] = await legacyConnection.execute('SELECT MAX(id) as maxId FROM users') as any[]
  if (maxId[0].maxId) {
    await prisma.$executeRawUnsafe(`ALTER TABLE User AUTO_INCREMENT = ${maxId[0].maxId + 1}`)
    console.log(`Set User AUTO_INCREMENT to ${maxId[0].maxId + 1}`)
  }

  console.log(`\nMigration completed! ${successCount} users migrated, ${errorCount} errors`)

  // Show final stats
  const totalUsers = await prisma.user.count()
  console.log(`Total users in new database: ${totalUsers}`)
